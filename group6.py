# group6.py - Run: python group6.py
import math
try:
...
def plot_cut_graph(stn_names,cut_N_vals,cut_E_vals,s_vals,name_P,N_P,E_P):
    ...
    # GROUP SIX: Dark navy theme
    colors=['#8e44ad','#16a085','#e91e8c','#2980b9','#d35400','#27ae60']
    ...
    ax.grid(which='minor',color='#c8e6c9',linewidth=0.4,linestyle='-')
    ax.xaxis.set_major_locator(ticker.MultipleLocator(x_step))
    ax.yaxis.set_major_locator(ticker.MultipleLocator(y_step))
    ax.grid(which='major',color='#81c784',linewidth=0.9,linestyle='-')
    ax.axhline(y=0,color='#333333',linewidth=1.2,zorder=3)
    ...
    ax.plot(conv_E,conv_N,'x',color='black',markersize=8,markeredgewidth=2.5,zorder=10,label='Convergence (P final)')
    ax.axhline(y=conv_N,color='#555555',linewidth=0.7,linestyle=':',zorder=3)
    ax.axvline(x=conv_E,color='#555555',linewidth=0.7,linestyle=':',zorder=3)
    ax.annotate(f"  P final\n  Cut N = {conv_N:.2f}\n  Cut E = {conv_E:.2f}",
                (conv_E,conv_N),textcoords="offset points",xytext=(10,-28),fontsize=9,fontweight='bold',color='black',
                bbox=dict(boxstyle='round,pad=0.4',facecolor='#ffffcc',alpha=0.95,edgecolor='black'))
...
def main():
    sep()
    print("  WELCOME TO GROUP SIX'S SURVEY COMPUTATION SYSTEM".center(72))
    print("  GROUP SIX SURVEY CUTS".center(72))
    sep()
    print()
    name_P=input("  What's the name of the unknown point? ").strip() or "P"
    sep("KNOWN STATIONS")
    print("  We will compute provisional coordinates of the unknown station")
    print("  from intersection (bearing method).\n")
    n_total=get_int("  How many stations do you have? (at least 2): ",minimum=2)
    stations=[]
    for i in range(n_total):
        print(f"\n  Station {i+1}:")
        stn_name=input(f"    Station name: ").strip() or f"Stn{i+1}"
        N_C=get_float(f"    Give me the Northing of {stn_name}: ")
        E_C=get_float(f"    Give me the Easting of {stn_name}: ")
        brg=get_dms(f"    What's the bearing from {stn_name} to {name_P}?")
    ...
    print(f"\n  Select two stations to compute provisional coordinates of {name_P}.")
    print(f"  The remaining stations will be used for cut adjustment.\n")
    while True:
        idx_A=get_int("  Pick your first station:  ",minimum=1)-1
        idx_B=get_int("  Pick your second station: ",minimum=1)-1
